...
//
//  Modified by George Nascimento, September 2020 to work on ESP32 boards

#ifndef _____EEG_Poty_ESP32_Library_Definitions_h
#define _____EEG_Poty_ESP32_Library_Definitions_h

//=============================================================================
// COMMUNICATION MODE: WiFi UDP Access Point
//=============================================================================
// Access Point Configuration
#define AP_SSID     "Potyplex-EEG"         // Network name created by ESP32
#define AP_PASSWORD "eeg12345"             // Password (min 8 chars, or "" for open)
#define AP_CHANNEL  1                       // WiFi channel (1-13)
#define AP_MAX_CONNECTIONS 2                // Max simultaneous clients

// UDP Port - ESP32 listens on this port, clients connect to it
#define UDP_PORT 12345

// Device name (for mDNS discovery)
...
// DEBUG MODE
//=============================================================================
#define DEBUG 0  // if DEBUG==1 the data is streamed through USB serial for debugging

// SerialPort macro - for debug output only (data goes via UDP)
#if DEBUG == 1
  #define SerialPort Serial
#else
  #define SerialPort Serial  // Keep Serial for debug messages, UDP for data
#endif
...
//specific for FireBeatle ESP32 board
#define DRDY_pin   26
#define RESET_pin  13
#define PWDN_pin   27
#define START_pin  25
...
const byte spi_WAKEUP = 0b00000010;     // Wake-up from standby mode
const byte spi_STANDBY = 0b00000100;   // Enter Standby mode
const byte spi_RESET = 0b00000110;   // Reset the device
const byte spi_START = 0b00001000;   // Start and restart (synchronize) conversions
const byte spi_STOP = 0b00001010;   // Stop conversion
const byte spi_RDATAC = 0b00010000;   // Enable Read Data Continuous mode (default mode at power-up) 
const byte spi_SDATAC = 0b00010001;   // Stop Read Data Continuous mode
const byte spi_RDATA = 0b00010010;   // Read data by command; supports multiple read back
...
//PIN CONNECTIONS
#define ADS_DRDY 26          // ADS data ready pin
#define ADS_RST	13           // ADS reset pin
#define BOARD_ADS	5         // ADS chip select
...
#define SCALE_4G		0x08  //(b00001000)	// +/- 4G sensitivity
#define SCALE_8G		0x10  //(b00010000)	// +/- 8G sensitivity
#define SCALE_16G		0x18  //(b00011000)	// +/- 16G sensitivity
