...
*/
#ifndef _____EEG_Poty_ESP32__
#define _____EEG_Poty_ESP32__
...
private:
  ...
  void    MPU6050_writeAxisDataSerial(void);
  void    LIS3DH_writeAxisDataForAxisSerial(uint8_t);
  void    MPU6050_updateAxisData(void);
  ...
  void    START();   // start data acquisition
  void    STOP(int);    // stop data acquisition
  void    WAKEUP(int);  // get out of low power mode
  void    WREG(byte,byte);       // write one ADS register
  void    WREGS(byte,byte);      // write multiple ADS registers
